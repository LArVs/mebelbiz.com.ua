...
class modTemplateVarOutputRenderSHKRadio extends modTemplateVarOutputRender {
    public function process($value,array $params = array()) {
        ...
        foreach ($lines as $line) {
            ...
            $o .= "\n".$otag.'<input class="'.$params['cssclass'].'" type="radio" name="'.$params['param_name'].'__'.$params['id'].'" value="'.$cnt.'__'.$itemvalue.'" id="'.$params['param_name'].$params['id'].$cnt.'"'.$selected.' onclick="'.$params['function'].'" /> <label for="'.$params['param_name'].$params['id'].$cnt.'">'.$item.'</label>'.$ctag;
            $cnt++;
        }
