...
if(count($fields)>0 && count($fields) >= $minCount){
    ...
    foreach($fields as $key => $val){
        ...
        foreach($row as $k => $v){
            if(!empty($v) || !$noEmpty){
                ...
                $rowArr[$index]['field'.($separateCols ? '1' : ($k+1))] = $v;
                ...
                if(!isset($rowArr[$index]['inner'])) $rowArr[$index]['inner'] = '';
                ...
                if(!empty($rowChunk[2])){
                    ...
                    $rowArr[$index]['inner'] .= $chunk->process($colArr);
    ...
    if(strlen($out_arr[0])>0){
        ...
        $chunk->setCacheable(false);
        $out_arr[0] = $chunk->process(array('inner'=>$out_arr[0]));
        $output = $out_arr[0];
