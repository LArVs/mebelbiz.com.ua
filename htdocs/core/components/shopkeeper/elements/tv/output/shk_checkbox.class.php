...
class modTemplateVarOutputRenderSHKCheckbox extends modTemplateVarOutputRender {
    public function process($value,array $params = array()) {
        ...
        foreach ($lines as $line) {
            ...
            $o .= "\n".$otag.'<input class="'.$params['cssclass'].'" type="checkbox" name="'.$params['param_name'].'__'.$params['id'].'__'.$cnt.'" value="'.$cnt.'__'.$itemvalue.'" id="'.$params['param_name'].$params['id'].$cnt.'"'.$selected.' onclick="'.$params['function'].'" /> <label for="'.$params['param_name'].$params['id'].$cnt.'">'.$item.'</label>'.$ctag;
            $cnt++;
        }
